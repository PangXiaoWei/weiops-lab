...
import { isVoiceEnabled, opsAudioPath, playAudioOrTTS, setVoiceEnabled, stopAudio } from "./utils/audio";
...
function TerminalBlock({ commandsText, title = "模拟终端" }) {
  ...
  return <section className="terminal">
    <header><span><i></i><i></i><i></i>{title}<SpeakButton text={commandsText} id={`terminal-${title}`} /></span><div><button onClick={() => copy(commandsText, setMessage)}>复制命令</button><button onClick={() => setOpen(!open)}>{open ? "收起" : "展开"}</button></div></header>
...
function SpeakButton({ text, id, slow = false, label = "🔈" }) {
  if (!text) return null;
  return <button
    type="button"
    className="speak-button"
    title={slow ? "Slow American pronunciation" : "American pronunciation"}
    aria-label={slow ? "Slow American pronunciation" : "American pronunciation"}
    onClick={(event) => {
      event.preventDefault();
      event.stopPropagation();
      playAudioOrTTS({ audioPath: opsAudioPath(id || text), text, slow });
    }}
  >{label}</button>;
}

function EnglishWithAudio({ text, id, slow = true, className = "" }) {
  return <span className={`english-with-audio ${className}`}>
    <span>{text}</span>
    <SpeakButton text={text} id={id || text} />
    {slow && <SpeakButton text={text} id={`${id || text}-slow`} slow label="🐢" />}
  </span>;
}
...
function VoiceToggle() {
  ...
  return <button
    type="button"
    className={enabled ? "voice-toggle" : "voice-toggle muted"}
    onClick={() => {
      const next = !enabled;
      setEnabled(next);
  ...
  >{enabled ? "🔊 English Voice" : "🔇 Voice Off"}</button>;
}
...
function App() {
  ...
  useEffect(() => { const sync = () => { stopAudio(); setPage(location.hash.slice(1) || "home"); }; addEventListener("hashchange", sync); return () => removeEventListener("hashchange", sync); }, []);
  const navigate = id => { stopAudio(); location.hash = id; };
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topline"><span>从 Linux 故障排查到 DevOps 的实战训练站</span><div className="topline-actions"><VoiceToggle /><button className="quiz-trigger" onClick={() => setQuizOpen(true)}>开始一题测验</button></div></header>{content[page] || content.home}</main>
...
function Home({ navigate, progress, openQuiz }) {
  ...
  return <><section className="hero mission-hero"><div><p className="eyebrow"><EnglishWithAudio text="OPS MISSION CONTROL" id="dashboard-ops-mission-control" slow={false} /></p><h1><EnglishWithAudio text="WeiOps Lab" id="dashboard-weiops-lab" slow={false} /></h1><h2><EnglishWithAudio text="Professional incident training dashboard" id="dashboard-professional-incident-training-dashboard" /></h2><p className="hero-text">把 Linux、Docker、K3s、英文工单和实验记录组织成每天可执行的运维任务。</p><div className="hero-actions"><button onClick={() => navigate("troubleshooting")}>Start Incident Mission</button><button className="secondary" onClick={openQuiz}>Run Quick Check</button></div></div><TerminalBlock title="Mission bootstrap" commandsText={"systemctl status nginx\njournalctl -u nginx -n 50\ncurl -I http://127.0.0.1:8080"} /></section>
    <section className="ops-dashboard">
      ...
      <article className="mission-card primary"><p><EnglishWithAudio text="MISSION MODE" id="mission-mode" slow={false} /></p><h3>故障任务入口</h3><button onClick={() => navigate("troubleshooting")}>Start Incident Mission</button><small>从现象、命令、日志和英文描述完成一次排障闭环。</small></article>
      <article className="mission-card"><p><EnglishWithAudio text="ERROR CARDS" id="error-cards" slow={false} /></p><h3>已解锁错误 {errorUnlocked} / 20</h3><ProgressBar value={errorUnlocked} max={20} /><small>通过故障场景和测验逐步收集常见错误卡片。</small></article>
      <article className="mission-card"><p><EnglishWithAudio text="ENGLISH SPEAKING DRILL" id="english-speaking-drill" slow={false} /></p><h3>今日跟读 {speakingToday} / 5</h3><ProgressBar value={speakingToday} max={5} /><small>从工单表达里挑选英文句子，进行跟读和复述。</small></article>
      <article className="mission-card level-card"><p><EnglishWithAudio text="PROGRESS LEVEL" id="progress-level" slow={false} /></p><h3><EnglishWithAudio text={level.name} id={`level-${level.name}`} /></h3><ProgressBar value={level.pct} max={100} /><small>完成任务越多，等级越接近真实 DevOps 工作流。</small></article>
    </section>
...
function Troubleshooting({ refresh }) {
  ...
  return <><ModuleTitle kicker="INCIDENT RESPONSE" title="Linux 故障排查"><p>先形成排障顺序，再记住命令。</p></ModuleTitle><div className="filters"><select value={difficulty} onChange={event => setDifficulty(event.target.value)}>{["全部","Beginner","Intermediate","Advanced"].map(item => <option key={item}>{item}</option>)}</select>{tags.map(item => <button className={tag === item ? "selected" : ""} key={item} onClick={() => setTag(item)}>{item}</button>)}</div><section className="scenario-grid">{visible.map(scenario => <article className="scenario-card" key={scenario.id}><header><div><Tag kind={scenario.difficulty === "Beginner" ? "green" : scenario.difficulty === "Advanced" ? "red" : "amber"}>{scenario.difficulty}</Tag><h3>{scenario.title}</h3></div><button className={mastered.includes(scenario.id) ? "complete active" : "complete"} onClick={() => mark(scenario.id)}>{mastered.includes(scenario.id) ? "已掌握" : "标记掌握"}</button></header><p className="symptom">{scenario.symptom}</p><p><b>问题：</b>{scenario.question}</p><div className="options">{scenario.options.map((option, index) => <button key={option} className={answers[scenario.id] !== undefined ? (index === scenario.correct ? "right" : index === answers[scenario.id] ? "wrong" : "") : ""} onClick={() => setAnswers({ ...answers, [scenario.id]: index })}>{"ABCD"[index]}. {option}</button>)}</div>{answers[scenario.id] !== undefined && <div className={answers[scenario.id] === scenario.correct ? "feedback good" : "feedback bad"}><b>{answers[scenario.id] === scenario.correct ? "思路正确" : "再想一想"}</b><p>{scenario.explanation}</p></div>}<TerminalBlock commandsText={scenario.commandsText} title="推荐排查命令" /><p className="english-line"><EnglishWithAudio text={scenario.english} id={`scenario-english-${scenario.id}`} /></p></article>)}</section></>;
}

function Network() { const flow = ["能 ping 通服务器吗？","DNS 能解析吗？","端口是否开放？","服务是否监听？","本机 curl 是否成功？","防火墙是否阻止？","日志是否有错误？"]; const ticketOne = "I checked DNS resolution, network connectivity, and port availability."; const ticketTwo = "The server was reachable, but the application port was not listening."; return <><ModuleTitle kicker="NETWORK TROUBLESHOOTING" title="网络诊断"><p>面对“网站打不开”，按顺序排查，不跳步骤。</p></ModuleTitle><section className="flow-card"><div><p><EnglishWithAudio text="CASE FLOW" id="case-flow" slow={false} /></p><h2>网站无法访问</h2></div><ol>{flow.map((step, index) => <li key={step}><span>{String(index + 1).padStart(2,"0")}</span><strong>{step}</strong></li>)}</ol></section><section className="two-column"><TerminalBlock title="网络诊断工具箱" commandsText={"ip addr\nip route\nping -c 4 server\ndig example.com\nss -lntp\ncurl -I http://127.0.0.1:8080\nfirewall-cmd --list-all"} /><article className="info-card"><Tag>WORK TICKET <SpeakButton text="WORK TICKET" id="work-ticket" /></Tag><h3>英文表达</h3><p><EnglishWithAudio text={ticketOne} id="network-ticket-one" /></p><p><EnglishWithAudio text={ticketTwo} id="network-ticket-two" /></p></article></section></> }
